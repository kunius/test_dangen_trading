...
def on_tick(context,tick):
    ...
    if context.find_index != 0:
        #三个tick 执行一次
        context.query_tick += 1
        if context.query_tick == 1:
            context.query_tick_bar['high'] = tick['price']
            context.query_tick_bar['low'] = tick['price']
        else:
            if tick['price'] > context.query_tick_bar['high']:
                context.query_tick_bar['high'] = tick['price']
            if tick['price'] < context.query_tick_bar['low']:
                context.query_tick_bar['low'] = tick['price']

        if context.query_tick != 3:
            return
        ...
        if positions: # 如果存在仓位
            ...
            if positions[0].side == PositionSide_Long: # 多单
                if (context.query_tick_bar['high'] >= positionPrice+winPoint) or (context.query_tick_bar['low'] <= positionPrice-lostPoint): #止损止盈
                    print("平多单前")
                    ...
                    if context.query_tick_bar['high'] >= positionPrice+winPoint:
                        vv_backtest.utils.updateInfoToJson(configPath, tradeResultKey, 1)
                    elif context.query_tick_bar['low'] <= positionPrice-lostPoint:
                        vv_backtest.utils.updateInfoToJson(configPath, tradeResultKey, 0)
                    trade_count_add()
                    print("平完多单")
            else: # 空单
                if (context.query_tick_bar['high'] >= positionPrice+lostPoint) or (context.query_tick_bar['low'] <= positionPrice-winPoint): #止损止盈
                    print("平空单前")
                    # order_target_percent(symbol=context.symbol, percent=0, order_type=OrderType_Market,
                    #                      position_side=PositionSide_Long)  #平仓
                    order_target_percent(symbol=context.symbol, percent=0, order_type=OrderType_Market,
                                         position_side=PositionSide_Short)  #平仓
                    if context.query_tick_bar['high'] >= positionPrice+lostPoint:
                        vv_backtest.utils.updateInfoToJson(configPath, tradeResultKey, 0)
                    elif context.query_tick_bar['low'] <= positionPrice-winPoint:
                        vv_backtest.utils.updateInfoToJson(configPath, tradeResultKey, 1)
                    trade_count_add()
                    print("平完空单")
...
def trade_count_add():
    ...
    if tradeCount > 99999:
        tradeCount = 0
    strCount = str(tradeCount)
    strCountLen = len(strCount)
    if strCountLen == 1:
        ...
        strCount = "0" + strCount
    vv_backtest.utils.updateInfoToJson(configPath, tradeCountKey, strCount)

if __name__ == '__main__':
    run(strategy_id='3dfcba6c-e03e-11e9-8ee1-00ff5e0b76d41',
        filename='main2_slave.py',
        mode=MODE_BACKTEST,
